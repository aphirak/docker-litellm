...
DB_DIR=${LITELLM_DB_DIR:-/var/lib/postgresql}
SHARED_DIR=${LITELLM_SHARED_DIR:-/var/lib/litellm-secrets}
PASSWORD_FILE="$SHARED_DIR/postgres_password"
...
write_password() {
  tmp_file=$(mktemp "$SHARED_DIR/.postgres_password.XXXXXX")
  printf '%s\n' "$1" > "$tmp_file"
